...
try {
    ...
    $sql = "
        ...
        " . ($actor['role'] === 'member' ? " AND ar.user_id = :self_user_id" : "") . "
        ORDER BY ar.scan_datetime DESC
    ";

    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':club_id', $clubId, PDO::PARAM_INT);
    if ($actor['role'] === 'member') $stmt->bindValue(':self_user_id', $actor['user_id'], PDO::PARAM_INT);
    $stmt->execute();
    ...
    foreach ($records as &$record) {
        ...
        $record['slot_id'] = $record['slot_id'] ? (int)$record['slot_id'] : null;
