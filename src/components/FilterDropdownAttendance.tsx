...
export interface AttendanceFilters {
  search: string;
  statuses: { present: boolean; late: boolean; absent: boolean; excused: boolean };
  eventTypes: { event: boolean; activity: boolean };
  course: string;
  year: string;
  section: string;
}

interface FilterDropdownAttendanceProps {
  isOpen: boolean;
  toggle: () => void;
...
const FilterDropdownAttendance: React.FC<FilterDropdownAttendanceProps> = ({
  isOpen, toggle, filters, setFilters,
  courses, years, sections,
}) => {
  const toggleStatus = (status: keyof AttendanceFilters['statuses']) =>
    setFilters(f => ({
      ...f,
      statuses: { ...f.statuses, [status]: !f.statuses[status] },
    }));

  const toggleEventType = (type: keyof AttendanceFilters['eventTypes']) =>
    setFilters(f => ({
      ...f,
      eventTypes: { ...f.eventTypes, [type]: !f.eventTypes[type] },
    }));
    ...
    <div className="relative inline-block text-left dark:text-white">
      ...
      >
        ...
        <div className="px-4 py-2 border-t dark:border-gray-600">
          <p className="text-xs font-medium mb-1 dark:text-gray-300">Event Type</p>
          {(['event', 'activity'] as const).map(type => (
